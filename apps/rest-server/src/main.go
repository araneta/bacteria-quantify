...
func main() {
	...
	//di testing
	svcProvider := InitializeServiceProvider(c)

	//admin controller
	adminCont.ServiceProvider = svcProvider
	...
	API := app.Party("/api")
	{
		...
		//logged in user
		API.Post("/user/avatar", j.Serve, userCont.UploadAvatar)
		API.Get("/user/profile", j.Serve, userCont.GetProfile)
		API.Post("/user/profile", j.Serve, userCont.UpdateProfile)

		//start here
		API.Get("/admin/users/search", j.Serve, adminCont.SearchUsers)
		API.Post("/admin/users", j.Serve, adminCont.AddUser)
